...
class Transform2D extends ChangeNotifier {
  ...
  // TODO(sign): factory Transform2D.copy(Transform2D other) => Transform2D()
  //   ..angle = other.angle
  ...
  /// Clone of this.
  // TODO(sign): Transform2D clone() => Transform2D.copy(this);
  ...
  bool closeTo(Transform2D other, {double tolerance = 1e-10}) {
    final deltaAngle = (angle - other.angle) % _tau;
    assert(deltaAngle >= 0);
    return (deltaAngle <= tolerance || deltaAngle >= _tau - tolerance) &&
  ...
  double get angleDegrees => _angle * (360 / _tau);
